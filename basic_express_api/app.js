...
app.get('/animaltypes', (req, res) => {
    // let zoos = res.json(zoos);
    // let animals = zoos['animals.name'];
    // let animal = animals[req.params.name];
    // res.json(animal);
    // .filter 
    let typesOfAnimals = [];
    zoos.forEach(zoo => {
        typesOfAnimals.push(zoo.animals.map(x => x.name));
    });

    res.json({'types': typesOfAnimals});
});
